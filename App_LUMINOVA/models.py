...
class CategoriaProductoTerminado(models.Model):
    nombre = models.CharField(max_length=100, unique=True, verbose_name="Nombre Categoría PT") # Aumentado max_length
...
class CategoriaInsumo(models.Model):
    nombre = models.CharField(max_length=100, unique=True, verbose_name="Nombre Categoría Insumo") # Aumentado max_length
...
class ComponenteProducto(models.Model): # NUEVO: Bill Of Materials (BOM)
    ...
    insumo = models.ForeignKey(Insumo, on_delete=models.PROTECT) # PROTECT para no borrar insumos si se borra el componente
...
class OrdenVenta(models.Model): # NUEVO: Específico para Ventas
    ...
    def actualizar_total(self):
        ...
        if self.total_ov != nuevo_total:
            ...
            self.save(update_fields=['total_ov']) # Solo guarda el campo total para evitar recursión
...
class EstadoOrden(models.Model): # Renombrar a EstadoOrdenProduccion o similar si es solo para OP
    ...
    nombre = models.CharField(max_length=50, unique=True) # Ej: Pendiente, En Proceso, Terminado
    def __str__(self):
        return self.nombre

class SectorAsignado(models.Model): # También, si es para OP, `SectorProduccion` sería más claro.
    nombre = models.CharField(max_length=50, unique=True) # Ej: Grupo A, Taller Principal
    def __str__(self):
        return self.nombre
...
class Factura(models.Model):
    ...
    fecha_emision = models.DateTimeField(default=timezone.now) # Cambiado a DateTimeField
    total_facturado = models.DecimalField(max_digits=12, decimal_places=2) # Aumentado max_digits
    def __str__(self):
